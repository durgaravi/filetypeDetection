main2 <- function(mimetype)
{
	...
	##############################
	dataset <- loadAndProcess(tnpath='train.hist', vpath='val.hist', tspath='test.hist')
	X <- dataset[[1]]
	y <- dataset[[2]]
	Xval <- dataset[[3]]
	yval <- dataset[[4]]
	...
	#output the model
	exportNNParams(outputfile='tika.model',nn_params, 256, hidden_layer_size, 1, testcost,gsub("-","/",mimetype))
}
